...
with open('./text_classification-master/text classification/stop/stopword.txt',  'rb') as f:
    STOP_WORDS = [line.strip() for line in f.readlines()]

# 加载停用词
#STOP_WORDS = [line.strip() for line in open('./text_classification-master/text classification/stop/stopword.txt', 'r', encoding='utf-8').readlines()]  
#STOP_WORDS  
...
def load_data(base_path):
    ...
    for root, dirs, files in os.walk(base_path): # 循环所有文件并进行分词打标
        for file in files:
            label = root.split('/')[-1] # win电脑用label = root.split('\\')[-1]因为windows上路径符号自动转成\了，所以要转义下
            labels.append(label)
